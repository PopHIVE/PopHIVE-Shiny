...
obesity_age_p <-  diab_obesity %>%
  filter(geography=='New York'  & !is.na(age_level) & outcome_name=='obesity') %>%
  ggplot(aes(x=age_level, y=Outcome_value1)) +
  geom_point()+
  ylab('Percent with obesity')+
  ylim(0,75) +
  theme_minimal()+
  ggtitle(paste0('Percent of population with obesity in', 'New York'))
...
obesity_diabetes_map <-  diab_obesity %>%
  ...
  ggtitle(paste("Diabetes prevalence (HbA1C>37) (all ages)"))
...
diabetes_age_p <-  diab_obesity %>%
   filter(geography=='New York'  & !is.na(age_level) & outcome_name=='diabetes') %>%
   ggplot(aes(x=age_level, y=Outcome_value1)) +
   geom_point()+
   ylab('Percent with diabetes')+
  ylim(0,42) +
   theme_minimal()+
   ggtitle('Percent of population with diabetes in New York')
