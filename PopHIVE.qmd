...
nrevss_rsv_ts <- read_csv(paste0(url_files,'rsv_ts_nrevss_test_rsv.csv')) 

rsv_all_indicators_state <- read_csv(paste0(url_files,'rsv_combined_all_outcomes_state.csv'))

flu_all_indicators_state <- read_csv(paste0(url_files,'flu_combined_all_outcomes_state.csv'))

covid_all_indicators_state <- read_csv(paste0(url_files,'covid_combined_all_outcomes_state.csv'))

rsvnet_age <- read_csv(paste0(url_files,'rsv_hosp_age_respnet.csv'))

epic_ed_combo <- read_csv(paste0(url_files,'rsv_flu_covid_epic_cosmos_age_state.csv'))

rsv_flu_covid_county_filled_nssp <- read_csv(paste0(url_files,'rsv_flu_covid_county_filled_map_nssp.csv'))
```
...
## Chronic disease, injury, and overdose
...
diab_obesity <- read_parquet('https://github.com/ysph-dsde/PopHIVE_DataHub/raw/refs/heads/repo_reorg/Data/Archive/Cosmos%20ED/diabetes_obesity.parquet')
...
obesity_age_p <-  diab_obesity %>%
  ...
  ylim(0,75) +
  theme_minimal()+
  ggtitle(paste0('Percent of population with obesity in ', input$state.select.obesity_epic ))
...
diabetes_age_p <-  diab_obesity %>%
  ...
  ylim(0,20) +
   theme_minimal()+
   ggtitle(paste0('Percent of population with diabetes in ', input$state.select.diabetes_epic))
...
s1 <- read.csv('https://raw.githubusercontent.com/ysph-dsde/PopHIVE_DataHub/refs/heads/main/Data/Pulled%20Data/youth_wellbeing/WISQARS/youth_self_harm.csv') %>%
...
g1_overdose <- read.csv('https://github.com/ysph-dsde/PopHIVE_DataHub/raw/refs/heads/main/Data/Archive/google_overdose/OD_search_state_recent12m.csv') %>%
  group_by(state) %>%
...
ggplotly(input$state.select.google_nal)
})
```
