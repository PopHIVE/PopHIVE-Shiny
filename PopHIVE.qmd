...
output$vax_urban_plot <- renderPlotly({
  ...
  plot_vax <- vax_urban %>% 
    ...
    ggplot(aes(x = Outcome_value1, y = urban, 
               text=paste0(Geography, "<br>", 
                           Vaccine_dose, "<br>",
                           Outcome_value1, "%" ))) +
    geom_jitter(aes( color = Outcome_value1), height = 0.1, width = 0, alpha = 0.7) + 
...
selectInput("state.select.obesity_epic", "State:", 
            choices=state.name, selected='New York', multiple=T)
```
...
s1 <- read.csv('https://github.com/ysph-dsde/PopHIVE_DataHub/raw/refs/heads/main/Data/Plot%20Files/wisqars_self_harm.csv') %>%
